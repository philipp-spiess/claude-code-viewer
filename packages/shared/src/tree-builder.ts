...
/**
 * Builds a linear message list with tool result linking, skipping tree structure
 */
export function buildLinearMessageList(messages: TranscriptMessage[]): MessageNode[] {
  // Create a map to store tool results by tool_use_id
  const toolResultMap = new Map<string, any>();
  ...
  for (const msg of messages) {
    if ("uuid" in msg && msg.uuid) {
      const node: MessageNode = {
        message: msg,
        children: [], // Always empty for linear structure
      };
